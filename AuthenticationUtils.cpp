...
namespace
{
    ...
    {
        if (SQLUtils::checkQueryCondition(databasePath, "SELECT ID FROM users WHERE username = ? LIMIT 1;", {username}))
        {
     ...
     */
    void addUsernameInDatabase(const std::string& username, const std::string& password, const std::string& databasePath)
    {
...
bool AuthenticationUtils::loginUser()
{
    ...
    if (!SQLUtils::checkQueryCondition("users.db", "SELECT ID FROM users WHERE username = ? LIMIT 1;", {username}))
    {
        std::cerr << "User \"" << username << "\" not found in database" << std::endl;
        return false;
    }

    if (SQLUtils::checkQueryCondition("users.db", "SELECT ID FROM users WHERE username = ? AND password = ? LIMIT 1;", {username, password}))
    {
